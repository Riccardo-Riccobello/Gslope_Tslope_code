...
#Codice per BoxPlot quando si simulano dati Tstudent o Mixture#
Box_Plot_sim <- function(output_performance){
  
  cond_plot <- output_performance$data_cond %>%
    ggplot( aes(x=factor(name, levels = c("Sample", "Elastic", "Rope",
                                          "Glasso", "Tlasso", "Gslope", "Tslope")),
                y=value, fill=factor(name, levels = c("Sample", "Elastic", "Rope",
                                                      "Glasso", "Tlasso", "Gslope", "Tslope")))) +
